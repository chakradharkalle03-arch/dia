"""
CPU Version: Fine-tune SpeechT5 model on NonverbalTTS dataset.
Optimized for CPU training with smaller batch sizes and efficient memory usage.
"""
import os
import json
import torch
...
def collate_fn(batch):
    ...
    for i in range(len(batch)):
        ...
        # Pad labels (variable length audio)
...
def main():
    ...
    output_dir = Path("checkpoints/speecht5_finetuned_cpu")
    ...
    print("=" * 70)
    print("Fine-tuning SpeechT5 on NonverbalTTS Dataset - CPU VERSION")
    print("=" * 70)
    print(f"\nDevice: {device}")
    ...
    print("\nNote: CPU training will be slower. Consider using GPU if available.")
