"""
GPU Version: Test the pretrained Dia model fine-tuned on NonverbalTTS dataset.
...
def load_trained_model(checkpoint_dir: str = "checkpoints/dia-nonverbal-lora-gpu/epoch_50"):
    """Load the trained model from checkpoint."""
    checkpoint_path = Path(checkpoint_dir)
    
    print("=" * 70)
    print("Loading Pretrained Model - GPU VERSION")
    print("=" * 70)
    ...
    if not checkpoint_path.exists():
        print(f"\nWarning: Checkpoint directory not found: {checkpoint_path}")
        print("Using base model instead...")
        checkpoint_path = None
    
    # Load base model
    print(f"\nLoading base model: nari-labs/Dia-1.6B-0626")
    compute_dtype = "float32"  # Use float32 for stability
...
def main():
    ...
    print("=" * 70)
    print("Dia Model - Pretrained Model Testing - GPU VERSION")
    ...
    output_path = f"generated_audio/test_output_gpu_{timestamp}.wav"
    ...
    if audio is not None:
        ...
        print(f"  2. Check that nonverbal tokens (laughs) are generated correctly")
